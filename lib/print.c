...
#define 	OUTPUT(arg, s, l)  \
  { if (((l) < 0) || ((l) > LP_MAX_BUF)) { \
    ...
    */
	...
	//determine ladjust(ladjust == 1 means left justifying)
	if (*fmt == '-') {
		ladjust = 1; fmt++;
	} else {
		ladjust = 0;
	...
	//determine padc(character in blank positon)
	if (*fmt == '0') {
		padc = '0'; fmt++;
	} else {
		padc = ' ';
	...
	//determine width(whether length of digital is longer than width or not)
	width = 0;
	while (IsDigit(*fmt)) {
	...
	if (*fmt == '.') {
		...
		prec = 6; // default precics is 6
	}

	/* check format flag */
	
	//determine longFlag(whether data type is "long" or not)
	if (*fmt == 'l') {
		longFlag = 1; fmt++;
	} else {
		longFlag = 0;
	 ...
	 int length, int ladjust, char padc, int upcase)
{
	...
	/* convert number to string, but the string is reversed*/
    do {
		int tmp = u % base;
		if (tmp <= 9) {
	    	*p++ = '0' + tmp;
