...
env_alloc(struct Env **new, u_int parent_id)
{
    ...
    e->env_runs = 0; //have not been run   
    e->env_thread_bitmap = 0;
    bzero(&e->env_threads, sizeof(e->env_threads));

    LIST_REMOVE(e, env_link);
    printf("\033[1;33;40m>>> env %d is alloced ... <<<\033[0m\n\n", e->env_id);
    // color_printf(1, 33, ">>> env %d is alloced <<<", e->env_id);
    *new = e;
    return 0;
}
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    if (offset > 0) {
        ...
        //determine the size and then copy;
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
    ...
    perm = PTE_R;
    ret = page_insert(e->env_pgdir, p, USTACKTOP - BY2PG, perm);
    if (ret < 0) return;
...
env_create_priority(u_char *binary, int size, int priority)
{
    ...
    struct Env *e;
    struct Thread *t;
    // Use env_alloc to alloc a new env. 
    ret = env_alloc(&e, 0);
    if (ret < 0) return;

    // assign priority to the new env.  
    e->env_pri = priority;

    // Use thread_alloc to alloc a new thread, whose index is 0;
    ret = thread_alloc(e, &t);
    if (ret < 0) return ret;

    // Use load_icode() to load the named elf binary,
