...
void mips_detect_memory()
{
	...
	basemem = 1 << 26; //0x04000000, 64KB
	maxpa = 0x4000000; //0x04000000
	extmem = 0;
	// Step 2: Calculate corresponding npage value.
	npage = 1 << 14; //basemem >> PGSHIFT
...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	...
	pgtable = (Pte *)KADDR(PTE_ADDR(*pgdir_entryp)); //find kernel !virtual! addr of second-level page table.
...
void page_init(void)
{
	...
	int mapped_cnt = PPN(PADDR(freemem));
	...
	for (i = mapped_cnt; i < npage; i++) {
		if (i == PPN(PADDR(TIMESTACK))) {
			pages[i].pp_ref = 1;
			continue;
		}
		pages[i].pp_ref = 0;
...
int page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
	...
	//if second pg doesn't exit or pgtable_entry is not raleted to a PA
	tlb_invalidate(pgdir, va);
